...
import DataTable, { type ActionMenuItem, type TableColumn } from "@/components/Templates/DataTable"
...
const Roles = () => {
    ...
    const roleColumns: TableColumn[] = [
        { key: "name", label: "Name", sortable: true, className: "font-medium text-gray-900" },
        {
            ...
            render: (item) => (
                <div>
                    {item.permissions.map((p: { id: string; action: string; subject: string }, i: number) => (
                        <p key={i}>
    ...
    const roleActions: ActionMenuItem[] = [
        {
            ...
            action: "update",
            subject: (item: Role) => (!item.isTemplate ? "Role" : ""),
        },
    ]
        ...
        <>
            <DataTable
                ...
                deleteConfig={{
                    ...
                    getDeleteDescription: (item: Role | null) =>
                        item
